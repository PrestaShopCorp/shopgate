...
<link rel="stylesheet" type="text/css" href="{$mod_dir|escape:'html':'UTF-8'}views/css/{$configCss}">
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        {foreach from=$shipping_service_list key=shipping_service_key item=shipping_service}
            <div class="form-group">
                <label class="control-label col-xs-2">{$shipping_service}</label>

                <div class="col-xs-10">
                    {assign var=carrier_mappings value=$shippingModel->getCarrierMapping()}
                    <select name="settings[SG_CARRIER_MAPPING][{$shipping_service_key}]">
