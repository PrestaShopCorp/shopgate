...
class ShopgateItemsCartExportJson extends ShopgateItemsCart
{
			...
			if (Validate::isLoadedObject($cartRule))
			{
				...
				switch ($cartRule->reduction_tax)
				{
					case 1 :
						...
						$result->setAmountNet($cartRule->getContextualValue(false, $this->getPlugin()->getContext()));
						break;
					case 0 :
						$result->setTaxType(Translate::getAdminTranslation('auto'));
	...
	protected function _getCarriers()
	{
		...
		{
			foreach (Carrier::getCarriersForOrder(Address::getZoneById($this->_deliveryAddress->id), null, $this->getPlugin()->getContext()->cart) as $carrier)
			{
				...
				if (version_compare(_PS_VERSION_, '1.5.0', '<'))
					$carrierTax = $carrierItem->getCarriersOpc($this->_deliveryAddress->id_country);
				else
