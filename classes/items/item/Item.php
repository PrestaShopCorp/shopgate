...
class ShopgateItemsItem extends ShopgateItemsAbstract
{
	...
	public function getItems($limit = null, $offset = null, array $uids = array())
	{
		...
		$productItems = Product::getProducts(
			$this->getPlugin()->getLanguageId(), $offset, $limit, 'id_product', 'DESC', false, true
		);
		...
		foreach ($productItems as $product)
		{
			...
			$this->addAdditionalInfo('images', $this->prepareImages(
				$this->currentProduct->getImages($this->getPlugin()->getLanguageId()))
			);
	...
	{
		/*if(!empty($this->currentProduct->tax_rate))
		{
			return (float) $this->currentProduct->tax_rate;
		}
		
		$tax = Tax::getDataByProductId($this->currentProduct->id);
		return (!empty($tax["rate"]) ? (float)$tax["rate"] : '');*/
	}
	...
	public function prepareTaxClass()
	{
		...
		else
		{
			...
			$id_tax_rules 	= (int)Product::getIdTaxRulesGroupByIdProduct($this->currentProduct->id, null);
			...
			foreach ($taxes as $tax)
				foreach ($tax_calculator->taxes as $calcesTaxes)
					if (!empty($tax['id_tax']) && $calcesTaxes->id == $tax['id_tax'])
						return $tax['name'];
		}
	...
	protected function prepareIdentifiers($product = false)
	{
		...
		if (property_exists($product, 'reference'))
		{
			$identifierItem = new Shopgate_Model_Catalog_Identifier();
	...
	protected function prepareStock()
	{
		...
		$availableForOrder = isset($this->currentProduct->available_for_order)
		&& $this->currentProduct->available_for_order != 1 ? false : true;
		
		$stockItem->setIsSaleable($this->currentProduct->checkQty(1) && $availableForOrder ? 1 : 0);
		
		$availableTest = $this->currentProduct->available_now != ''
			? $this->currentProduct->available_now
			: '';
		
		if ($availableTest == '')
			switch ($stockItem->getIsSaleable())
			{
				case true:
					$availableTest = $this->getModule()->l('In stock');
					break;
				default :
					$availableTest = $this->getModule()->l('Out of stock');
					break;
			}
		
		
		$stockItem->setAvailabilityText($availableTest);
		...
		foreach ($images as $image)
		{
			...
			$imageItem->setUrl(
				$this->getPlugin()->getContext()->link->getImageLink(
					$this->currentProduct->link_rewrite[$this->getPlugin()->getLanguageId()], $this->currentProduct->id.'-'.$image['id_image']
				)
			);
