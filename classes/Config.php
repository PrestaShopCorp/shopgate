...
class ShopgateConfigPrestashop extends ShopgateConfig
{
	...
	public function startup()
	{
		...
		{
			if (!Configuration::hasKey(self::DEFAULT_CONFIG_NAME))
			{
				$this->initDefaults();
				$this->store();
			}
	...
	public function store()
	{
		...
		Configuration::updateValue(self::DEFAULT_CONFIG_NAME, serialize($vars));
	}
	...
	{
		foreach (unserialize(Configuration::get(self::DEFAULT_CONFIG_NAME)) as $key => $value)
			$this->$key = $value;
	}
